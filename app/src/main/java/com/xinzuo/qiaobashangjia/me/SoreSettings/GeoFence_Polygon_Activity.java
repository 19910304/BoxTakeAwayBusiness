...
import com.xinzuo.qiaobashangjia.entities.point_set;
import com.xinzuo.qiaobashangjia.netService.NetService;
import com.xinzuo.qiaobashangjia.util.Const;
...
public class GeoFence_Polygon_Activity extends CheckPermissionsActivity
    ...
    private String id;
    private List<point_set> pointSets = new ArrayList<>();
...
//        标题信息
        ...
        mMapView = (MapView) findViewById(R.id.mp_geofence);
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_save://保存信息
                ...
                for (int j = 0; j < pointSets.size(); j++) {
                    ...
                    try {
                        jsonObject.put("lat", pointSets.get(j).getLat());
                        jsonObject.put("lng", pointSets.get(j).getLng());
                        pointsArray.put(jsonObject);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }
                Log.i("设置的坐标", pointsArray.toString());
                if (Valid.isNotNullOrEmpty(id) && pointsArray.length() != 0) {
                    netService.updDisList(Integer.parseInt(id), pointsArray, Hander_updDisList);
                } else {
                    Toast.makeText(this, "最少设置三个点", Toast.LENGTH_SHORT).show();
                }
...
//        tvGuide.setBackgroundColor(getResources().getColor(R.color.gray_dark));
...
//        }
        ...
        point_set point_set = new point_set();
        pointSets.clear();
        for (LatLng latLng : polygonPoints) {
            pointList.add(new DPoint(latLng.latitude, latLng.longitude));
            Log.i("point", latLng.latitude + "--" + latLng.longitude);
            point_set.setLat(String.valueOf(latLng.latitude));
            point_set.setLng(String.valueOf(latLng.longitude));
            pointSets.add(point_set);
        }
