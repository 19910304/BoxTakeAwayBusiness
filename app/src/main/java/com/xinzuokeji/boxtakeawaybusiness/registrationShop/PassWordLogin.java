...
public class PassWordLogin extends BaseActivity {
            ...
            case R.id.bt_login_sure://登录
                if (Valid.isPhone(accountNumber) && Valid.isNotNullOrEmpty(password)) {
                    if (isNetworkAvailable(this)) {
                        ...
                        netService.Login(accountNumber, password, moble(), GSApplication.getInstance().pushService.getDeviceId(), loginHandler);
                    } else {
                        showTip(getString(R.string.net_work_error), Toast.LENGTH_SHORT);
                    }
                } else {
    ...
    private void bindAccount(final String user_phone) {
        if (Valid.isNotNullOrEmpty(user_phone)) {
            GSApplication.getInstance().pushService.bindAccount(user_phone, new CommonCallback() {
                @Override
...
//                    bindAccount(appUtil.getUniqueID());
    ...
    private static class LoginHandler extends Handler {
        ...
        private LoginHandler(PassWordLogin activity) {
...
//                                            Log.i("uid", appUtil.getUniqueID());
                                                activity.bindAccount(GSApplication.getInstance().pushService.getDeviceId());
                                            }
                                        });
                                        activity.showTip(message, Toast.LENGTH_SHORT);
                                        try {
                    ...
                    case 1002:
                        activity.bt_login_sure.setClickable(true);
                        activity.showTip(activity.getString(R.string.net_work_error), Toast.LENGTH_SHORT);
                        break;
                    default:
                        break;
                }
