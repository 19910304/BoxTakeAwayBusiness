...
public class Utility {
    ...
    //把文字绘制成图像
    public static Bitmap getBitmapByText(String text, int dipW, int dipH, String frontColor, String backColor) {
//        int width = dip2px(GSApplication.getInstance(), dipW);
//        int height = dip2px(GSApplication.getInstance(), dipH);
        Bitmap bmp = Bitmap.createBitmap(dipW, dipH, Bitmap.Config.RGB_565);
        ...
        paint.setTextSize(dipW / 7 * 3);
        paint.setAntiAlias(true);
        paint.setFilterBitmap(true);

        //计算绘制文字时的起始点坐标
        float tx = (dipW - getFontlength(paint, text)) / 2;
        float ty = (dipH - getFontHeight(paint)) / 2 + getFontLeading(paint);
...
//                    Log.i("sssss", "华为");
//                    activity.getWindow().setStatusBarColor(activity.getResources().getColor(R.color.red));
                    //华为
                } else {
                    activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
