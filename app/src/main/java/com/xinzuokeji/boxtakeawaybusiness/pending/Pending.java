...
public class Pending extends Fragment implements View.OnClickListener, XListView.IXListViewListener {
    ...
    private RadioButton rdb_pending_new_order, rdb_pending_wait_distribution, rdb_pending_reminder, rdb_pending_refund;
    XListView xlvOrder;
...
//                    bt.setClickable(true);
    ...
    private static class SetshopPutForwardHandler extends Handler {
        ...
        public SetshopPutForwardHandler(Pending activity) {
                ...
                switch (msg.what) {
                    case 2000:
                        ...
                        if (msg.obj.toString().equals("[]")) {
                            ...
                            activity.xlvOrder.setVisibility(View.GONE);
                            // 无数据显示默认图
                            activity.ll_order_no.setVisibility(View.VISIBLE);
//                        新订单
                            if (activity.rdb_pending_new_order.isChecked()) {
                                activity.img_order_no.setBackgroundResource(R.mipmap.pending_new_order);
                            }
//                        待处理
                            if (activity.rdb_pending_wait_distribution.isChecked()) {
                                activity.img_order_no.setBackgroundResource(R.mipmap.pending_wait_distribution);
                            }
//                        催单
                            if (activity.rdb_pending_reminder.isChecked()) {
                                activity.img_order_no.setBackgroundResource(R.mipmap.pending_reminder);
                            }
//                        退单
                            if (activity.rdb_pending_refund.isChecked()) {
                                activity.img_order_no.setBackgroundResource(R.mipmap.pending_refund);
                            }
//                        Toast.makeText(getActivity(), "暂无数据", Toast.LENGTH_SHORT).show();
                        } else {
                            activity.xlvOrder.setVisibility(View.VISIBLE);
                            activity.ll_order_no.setVisibility(View.GONE);
                            activity.mDate.clear();
                            activity.mDate.addAll((Collection<? extends OrderInfo>) msg.obj);
...
//                    showTip(msg.obj.toString(), Toast.LENGTH_SHORT);
    ...
    //旧的，容易引起内存泄漏
    @SuppressLint("HandlerLeak")
    Handler showUntreatedOLD = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 2000:
